...
- 若后续任务到酒店信息和实时价格必须严格通过 `rgh` CLI 获取，结果是最新的
- 不使用其他浏览器查询工具和酒店查询工具
...
拿到酒店名称和日期后，立即调用酒店查询能力，执行 `rgh hotel-detail` 查询当前价格和取消政策，不要等所有信息都齐全再查。
...
拿到基本信息后，调用酒店查询能力，执行 `rgh search-hotels`。如果用户提到了风格偏好（「有设计感」「亲子」「带早餐」），先用 `rgh hotel-tags` 确认标签再搜索。
...
然后只问**一个**问题，根据回答再重新搜索。调整清楚后再调用酒店查询能力，执行 `rgh search-hotels` 重新搜索。
...
所有酒店查询、价格查询、详情查询、标签查询，统一通过 `rgh` CLI 直接处理。

| 需要做什么                             | 调用什么                                               |
| -------------------------------------- | ------------------------------------------------------ |
| 搜索候选酒店                           | `rgh search-hotels`                                      |
| 查询某家酒店详情、房型、价格、取消政策 | `rgh hotel-detail`                                       |
| 确认标签/品牌筛选条件                  | `rgh hotel-tags` → `rgh search-hotels`                    |
...
酒店查询能力通过 `rgh` CLI 直接接入。

**登录授权检查**（首次使用或 Token 失效时执行）
