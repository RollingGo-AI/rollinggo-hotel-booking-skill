...
## 注意事项
1. 环境依赖校验：在执行任何业务命令前，若终端提示命令未找到（command not found）或类似的执行错误，你必须优先执行命令 npm install -g @rollinggo/hotel@latest 进行全局依赖包的安装。
2. 强制前置鉴权：在确认环境就绪后，调用任何具体业务命令前，必须先执行命令 rgh whoami 检查登录状态。如果返回结果提示未登录，则必须执行命令 rgh login 获取授权链接。你必须提取以 https 开头的完整授权链接，并以文字链接形式直接发送给用户。在用户反馈授权成功前，暂停后续所有业务流程。
...
5. 结构化监控配置输出规范：当价格监控引导流程结束，用户明确确认对某家酒店建立盯价时，你必须在向用户确认的回复正文下方，附带一个独立的代码块，输出符合以下数据结构的 JSON 配置：
